...
/// \brief 描述ELF segment的简单数据结构
/// Simple data structure to describe an ELF segment
// TODO: information hiding
struct SegmentInfo
{
    /// 给这个segment取个人类可以理解的合适的名称
    std::string generateName();

    // segment 的数据，LLVM 变量存在这里
    llvm::GlobalVariable *Variable; 

    uint64_t StartVirtualAddress;
    uint64_t EndVirtualAddress;
    bool IsWriteable;   // 可写
    bool IsExecutable;  // 可执行
    bool IsReadable;    // 可读
...
public:
    ...
    Pointer(bool IsIndirect, uint64_t Value) : IsIndirect(IsIndirect),
                                               Value(Value) {}
